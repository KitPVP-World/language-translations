...
Get-ChildItem -Path $outputDir | Remove-Item -Force -Recurse -ErrorAction SilentlyContinue
...
if ( [string]::IsNullOrEmpty($env:repository_username))
{
    ...
    {
        Write-Error "ERROR: 'repository_username' is not set, and fallback 'REPO_KITPVP_USERNAME' is also not set or empty."
...
if ( [string]::IsNullOrEmpty($env:repository_password))
{
    ...
    {
        Write-Error "ERROR: 'repository_password' is not set, and fallback 'REPO_KITPVP_PASSWORD' is also not set or empty."
        exit 1 # Exit with an error
    }
}
...
$plainPassword = $env:repository_password
...
Catch
{
    Write-Error "ERROR: Download failed.`nUri: $downloadUrl`nOutput File: $outputFile`nError Details: $( $_.Exception.Message )"
